...
module.exports = core({
    modules: [__webpack_require__(3), __webpack_require__(4), __webpack_require__(5), __webpack_require__(6), __webpack_require__(8), __webpack_require__(9), __webpack_require__(10), __webpack_require__(11), __webpack_require__(13), __webpack_require__(14)],
    options: {
        kit: 'standard',
        browser: true
    }
...
module.exports = function (_ref) {
    ...
    var setState = function setState(replacement) {
        var newState = is.function(replacement) ? replacement(deepCopy(state)) : replacement;
        handler(newState);
    };
...
module.exports = function (_ref, global) {
    ...
    var build = function build(element, queue, ancestry, parentIndex, updateAncestry, componentIdentity) {
        ...
        for (var i = 0; i < childList.length; ++i) {
            ...
            var _key2 = '' + i;
            var child = build(childElement, queue, ancestry, _key2);
            // a key attribute in the child will override the default
            // array index as key.
            if (child.attributes && 'key' in child.attributes) {
                _key2 = child.attributes.key;
            }
            assert(!children[_key2], 'view.build : duplicate child key (note that text elements and elements with no key attribute are given their array index as key)', ancestry.f(), _key2);
            childOrder.push(_key2);
            children[_key2] = child;
        }
...
var _require = __webpack_require__(0),
    ...
    diff = _require.diff,
    is = _require.is,
    longestChain = _require.longestChain;
...
module.exports = function (_ref, global) {
    ...
    var isHosted = global.window.origin !== null && global.window.origin !== 'null';
    if (!isHosted) {
        currentPath = '';
    }
...
module.exports = function (_ref) {
    ...
    var register = function register() {
        ...
        if (is.regExp(path)) {
            var numGroups = new RegExp(path.toString() + '|').exec('').length - 1;
            store.push({
                keys: Array(numGroups).fill(0).map(function (_, i) {
                    return { name: i };
                }),
                pattern: path,
                handler: handler
